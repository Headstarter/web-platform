...
` + html + `
    ...
    var end_modal = "";
    var updates_done = $('input#imgInp').length + $('.list').length + $('.plain').length;
        ...
        $('input#imgInp').each (function (_) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    end_modal += ('<p class="success">Променихте student.cv.picture успешно.</p>')
                    updates_done -= 1;
                },
                error: function(response) {
                    end_modal += ('<p class="danger">Не променихте student.cv.picture: ' + response.status.toString () + '</p>')
    ...
    $('.list').each (function (_) {
        var code = [];
        var li = $(this).children('li:not(".add"):not(".hidden")');
        for (let i = 0 ; i < li.length ; i ++) {
            var inputs = $(li[i]).children('input, textarea');
            code[i] = {};
            for (let j = 0 ; j < inputs.length ; j ++) {
                code[i][$(inputs[j]).attr('name')] = $(inputs[j]).val();
            }
        }
        ...
        console.log(code, data_id, data_name);
